...
void display7Seg(void)
{
    ...
    // Note that writing to D4 will affect communication with CS43L22 chip.
    	...
    	// 0 - a,b,c,d,e,f,		| B4, D3, B6, B2, E8, B8
    	// 1 - e,f				| E8, B8
    	// 2 - a,b,g,e,d		| B4, D3, D7, E8, B2
    	// 3 - a,b,c,d,g		| B4, D3, B6, E8, D7
    	// 4 - b,c,f,g			| D3, B6, B8, D7
    	// 5 - a,c,d,f,g		| B4, B6, B2, B8, D7
    	// 6 - a,c,d,e,f,g		| B4, B6, B2, E8, D7, B8
    	// 7 - a,b,c			| B4, D3, B6
    	// 8 - a,b,c,d,e,f,g	| B4, D3, B6, B2, E8, B8, D7
    	// 9 - a,b,c,d,f,g  	| B4, D3, B6, B2, B8, D7
    	...
    	// Going to need to add a piece of code here that reads time in
...
void setAlarm(void)
{
	...
	//Make variable to hold the temporary alarm time -- Note the type variable differs from AlarmStruct
	RTC_TimeTypeDef		tempAlarmTime;
	tempAlarmTime.RTC_Hours = AlarmStruct.RTC_AlarmTime.RTC_Hours;
	tempAlarmTime.RTC_Minutes = AlarmStruct.RTC_AlarmTime.RTC_Minutes;
	tempAlarmTime.RTC_Seconds = AlarmStruct.RTC_AlarmTime.RTC_Seconds;

	//Before setting the alarm, disable it
    RTC_AlarmCmd(RTC_Alarm_A,DISABLE);
    //Increase the hour or minute of the alarm based on which button was clicked

    // figure out which button was pushed
    if(buttonState == ALARMHOURBUTTON) {
       //increment the hour
       addHour(1,&tempAlarmTime); //Need to write this function
    } else if (buttonState == ALARMMINBUTTON){
      //increment the minute
      addMin(1, &tempAlarmTime);
	}

    //Make the AlarmStruct time equal to the tempAlarmTime
    //NOTE -- GOING TO ADD A CONDITION ON WHEN THIS IS DONE - time set button
    AlarmStruct.RTC_AlarmTime.RTC_Hours = tempAlarmTime.RTC_Hours;
    AlarmStruct.RTC_AlarmTime.RTC_Minutes = tempAlarmTime.RTC_Minutes;
    AlarmStruct.RTC_AlarmTime.RTC_Seconds = tempAlarmTime.RTC_Seconds;

	//Then, pass alarm struct to RTC_SetAlarm
    RTC_SetAlarm(RTC_Format_BCD, RTC_Alarm_A, &AlarmStruct); //&AlarmStruct is of type RTC_AlarmTypeDef Pg.414
...
void configuration(void)
{
	...
	*/
	...
	GPIO_StructInit( &GPIOInitStruct );
	GPIOInitStruct.GPIO_Pin = GPIO_Pin_7 | GPIO_Pin_9; // we are going to use PB6 and PB9
